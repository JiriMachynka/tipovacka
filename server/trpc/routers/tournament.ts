...
export const tournamentRouter = router({
	...
	getMatches: privateProcedure.input(z.object({ tournamentId: z.number() })).query(async ({ input }) => await getTournamentMatches(input.tournamentId)),
	...
	getUserMatches: privateProcedure.input(z.object({ tournamentId: z.number() })).query(async ({ ctx, input }) => await getUserMatches(input.tournamentId, ctx.userId)),
	...
	getOverallTeams: privateProcedure.input(z.object({ tournamentId: z.number() })).query(async ({ input }) => await getTournamentOverallTeams(input.tournamentId)),
	getPlayerOverallTips: privateProcedure.input(z.object({ tournamentId: z.number() })).query(async ({ ctx, input }) => await getPlayerOverallTips(input.tournamentId, ctx.userId)),
