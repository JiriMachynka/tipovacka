...
export const tournamentRouter = router({
	getOwnerEmail: privateProcedure
		.input(z.object({ tournamentId: z.number() }))
		.query(async ({ input }) => await getOwnerEmail(input.tournamentId)),
	...
	getData: privateProcedure
		.input(z.object({ tournamentId: z.number() }))
		.query(async ({ ctx, input }) => await getAllTournamentData(ctx.userId, input.tournamentId)),
	...
	getPoints: privateProcedure
		.input(z.object({ tournamentId: z.number() }))
		.query(async ({ ctx, input }) => await getTournamentPoints(input.tournamentId, ctx.userId)),
